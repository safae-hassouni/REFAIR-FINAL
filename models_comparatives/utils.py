...
def load_dataset(args):
    ...
    if args.dataset=='nba':
    ...
    elif args.dataset == 'aminer_l':
        # multi-sens and multi-label (labels binarized)
        sens_attr = 'continent'
        name = 'LCC'
        
        edgelist = csv.reader(open(os.path.join(datapath, dataname, f"edgelist_{name}.txt")))
        edges = [list(map(int, line[0].split("\t"))) for line in edgelist]
        edges = np.array(edges)

        labels_file = csv.reader(open(os.path.join(datapath, dataname, f"labels_{name}.txt")))
        labels = [float(line[0].split("\t")[1]) for line in labels_file]
        labels = np.array(labels)

        sens_file = csv.reader(open(os.path.join(datapath, dataname, f"sens_{name}.txt")))
        sens = [[float(line[0].split("\t")[1])] for line in sens_file]
        sens = torch.FloatTensor(sens)
        ...
        feature = np.load(os.path.join(datapath, dataname, f"X_{name}.npz"))
        ...
        feature = torch.FloatTensor(feature)
        feature = torch.cat([feature, sens], dim=-1)
        sens = sens.squeeze()
        ...
        if label_number >= feature.shape[0] * 0.5:
            print('label_number is larger than node number! Set to 1000')
            label_number = 1000
...
def re_features(adj, features, K):
    ...
    edge_index, values = torch_sparse.from_scipy(adj)
