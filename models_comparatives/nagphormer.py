...
class NagPhormer(nn.Module):
    ...
    def forward(self, batched_data):
        ...
        return output # torch.log_softmax(output, dim=1)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_bias=None):
        ...
        def transform(x, linear):
            x = linear(x)
            return x.view(batch_size, -1, self.num_heads, self.att_size).transpose(1, 2)

        q, k, v = [transform(x, linear) for x, linear in zip((q, k, v), (self.linear_q, self.linear_k, self.linear_v))]
        ...
        out = out.transpose(1, 2).contiguous().view(batch_size, -1, self.num_heads * self.att_size)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_bias=None):
        ...
        x = x + self.self_attention_dropout(y)

        y = self.ffn_norm(x)
        y = self.ffn(y)
        x = x + self.ffn_dropout(y)
        return x
