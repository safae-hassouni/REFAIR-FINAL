...
parser.add_argument('--feat_norm', type=str, default='row',
                    ...
                    help='Méthode de normalisation des features')
...
parser.add_argument('--n_heads', type=int, default=1, help='Number of Transformer heads') # 8
...
for idx in range(epoch):
    ...
    reg=0.1*torch.abs(delta_s.view(-1, 1)  * probs).sum()
    #  avec normalisation de frobenius
    #reg=0.3 * torch.norm(delta_s.view(-1, 1)  * probs, p="fro").sum() 
    ...
    if new_metric > best_metric and (idx+1)>=100:
