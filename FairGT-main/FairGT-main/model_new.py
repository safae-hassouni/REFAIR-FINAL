...
class EncoderLayer(nn.Module):
    ...
    def forward(self,x, attn_bias=None):
        ...
        """ F_init=x
            ...
            X_trans = X_trans.view(X_trans.shape[0], 64)
            X_trans=X_trans.to_dense()
            F_init=F_init.to_dense()
            F_init = F_init[:, :64]
            #print("Shape of F_init before subtraction:", F_init.shape)  # Avant la soustraction
            #print("Shape of X_trans before subtraction:", X_trans.shape) 
            res=F_init - X_trans
            #ress= SparseTensor.from_dense(res)
            ress_dense = res.to_dense() 
            term2 = 0.5 * torch.norm(ress_dense, p='fro')**2 
