...
# Tes tensors
src,dst,id=g.edges(form='all', order='srcdst')
edge_index = torch.stack([src,dst],dim=0)
def normalized_laplacian_sparse(A_sparse: SparseTensor):
    """
    Compute symmetric normalized Laplacian from SparseTensor adjacency matrix.
    """

    # Coalescer la matrice d'adjacence pour éviter les doublons d'arêtes
    A_sparse = A_sparse.coalesce()

    num_nodes = A_sparse.size(0)

    # Calcul du degré des nœuds (somme sur les lignes)
    deg = A_sparse.sum(dim=1)  # taille [num_nodes]

    # Calcul de D^{-1/2}
    deg_inv_sqrt = deg.pow(-0.5)
    deg_inv_sqrt[deg_inv_sqrt == float('inf')] = 0.  # Pour éviter les infinis

    # Normalisation de la matrice d'adjacence : D^{-1/2} * A * D^{-1/2}
    A_norm = A_sparse.mul(deg_inv_sqrt.view(-1, 1)).mul(deg_inv_sqrt.view(1, -1))

    # Créer la matrice identité (matrice diagonale sparse)
    I = SparseTensor.eye(num_nodes, device=A_sparse.device())

 
 # Convertir la matrice d'identité en densité pour effectuer l'opération correctement
    I_dense = I.to_dense()
    A_norm_dense = A_norm.to_dense()

    # Calcul du Laplacien : L = I - A_norm
    L_dense = I_dense - A_norm_dense

    # Convertir à nouveau en matrice creuse
    L_sparse = SparseTensor.from_dense(L_dense)
...
# Création de la matrice d'adjacence sparse
A = SparseTensor(
    row=edge_index[0],
    col=edge_index[1],
...
L = normalized_laplacian_sparse(A)
print(L)
...
for idx in range(epoch):
    ...
    #logits,s_loss = model(g.ndata['feat'],L)
    logits=model(g.ndata['feat'])
    probs = torch.softmax(logits, dim=1)         # [N, num_classes]
    delta_s = calculDS(sens)  
    reg=0.3*torch.abs(delta_s.view(-1, 1)  * probs).sum()
    #reg=0.3 * torch.norm(delta_s.view(-1, 1)  * probs, p="fro").sum() #  avec normalisation de frobenius
    ####
    ####
    X_trans = g.ndata['feat']
   # print(g.ndata['feat'].shape)
    F_init=logits
    lambda_s=0.3
    L_dense = L.to_dense() 
    #print(F_init.shape) #print(L_dense.shape)#print(X_trans.shape)
    F_init = F_init.view(F_init.shape[0], -1)
    
    term1 = (lambda_s / 2) * torch.trace(F_init.T @ L_dense @ F_init)
    # Calcul du terme erreur: 1/2 * || F - X_trans ||_F^2
    X_trans = X_trans.mean(dim=1)
    X_trans = X_trans.view(1000, 29)
    #X_trans = X_trans.view(X_trans.shape[0], -1)

    X_trans=X_trans.to_dense()
    F_init=F_init.to_dense()
    F_init = F_init[:, :64]
    projector = torch.nn.Linear(29, 2).to(X_trans.device)
    X_proj = projector(X_trans)
    resss = F_init - X_proj
    #ress= SparseTensor.from_dense(res)
    ress_dense = resss.to_dense() 
    term2 = 0.5 * torch.norm(resss, p='fro')**2 
    total_loss = term1 + term2
    
    loss = F.cross_entropy(logits[idx_train], labels[idx_train])+total_loss+reg
    loss.backward()
    optimizer.step()
    model.eval()
